...
if mouse_check_button_pressed(mb_left){
	...
	if (fire_cooldown <= 0 && !isReloading && mag_current > 0 && !is_meleeing){
		is_shooting = true
		alarm[0] = 10
...
if(mouse_check_button_pressed(mb_right) && !is_meleeing){
	state = states.melee
	is_meleeing = true
	alarm[2] = 30
	image_index=0
	...
	if(distance_to_object(obj_enemy) <= 30){
		instance_nearest(x,y,obj_enemy).hp -= global.zombie_health * .66 //why doesn't this kill them?	
	}
...
if(isReloading){
	show_debug_message("Is reloading: " + string(isReloading))
	show_debug_message("RELOADING")
	sprite_index = spr_player_reloading	
	image_speed = global.reload_speed
}else if(is_shooting){
	show_debug_message("Is shooting: " + string(is_shooting))
	show_debug_message("SHOOTING")
	sprite_index = spr_player_shooting	
	image_speed = 1
}else if(is_meleeing){
	show_debug_message("Is meleeing: " + string(is_meleeing))
	show_debug_message("MELEEING " + string(image_index))
	sprite_index = spr_player_melee
	image_speed = 1
}else{
	sprite_index = (state == states.walking) ? spr_player_walkin : spr_player
	//show_debug_message("WALKING/IDLE")
	image_speed = 1
}
